...
<nav class="{{$apply_theme->value('nav')}}">
    ...
    <ul class="navbar-nav ml-auto">
        <li class="nav-item dropdown">
            <a class="nav-link">
                <div id="clock">
                    <b>{{\Carbon\Carbon::now()->setTimezone($app_timezone)->format($app_s->clock_format)}}</b>
                </div>
            </a>
        </li>
    </ul>
    ...
    <ul class="navbar-nav ml-auto">
        ...
        <li class="nav-item dropdown">
            <a class="nav-link" data-toggle="dropdown" href="#">
                @if($auth_user->language == 'en')
                <i class="flag-icon flag-icon-us"></i>
                @elseif($auth_user->language == 'pt_BR')
                <i class="flag-icon flag-icon-br"></i>
                @endif
            </a>
            <div class="dropdown-menu dropdown-menu-right p-0">
                <a href="{{route('lang.update', 'en')}}" class="dropdown-item @if($auth_user->language == 'en') active @endif">
                    <i class="flag-icon flag-icon-us mr-2"></i> English
                </a>
                <a href="{{route('lang.update', 'pt_BR')}}" class="dropdown-item @if($auth_user->language == 'pt_BR') active @endif">
